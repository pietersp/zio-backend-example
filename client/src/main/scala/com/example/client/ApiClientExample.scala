...
object ApiClientExample extends ZIOAppDefault {
  ...
  val exampleProgram: ZIO[ApiClient, Throwable, Unit] =
    for {
      ...
      departmentId <- client
        ...
        .catchAll(error =>
          Console.printLine(s"Failed to create department: $error") *> ZIO.fail(
            new RuntimeException("Department creation failed")
          )
        )
      ...
      employeeId <- client
        ...
        .catchAll(error =>
          Console.printLine(s"Failed to create employee: $error") *> ZIO.fail(
            new RuntimeException("Employee creation failed")
          )
        )
      ...
      employee <- client
        ...
        .catchAll(error =>
          Console.printLine(s"Employee not found: $error") *> ZIO.fail(
            new RuntimeException("Employee fetch failed")
          )
        )
      ...
      phoneId <- client
        ...
        .catchAll(error =>
          Console.printLine(s"Failed to create phone: $error") *> ZIO.fail(
            new RuntimeException("Phone creation failed")
          )
        )
      ...
      _ <- client
        .addPhoneToEmployee(employeeId, phoneId)
        .catchAll(error =>
          Console.printLine(s"Failed to add phone to employee: $error") *> ZIO
            .fail(new RuntimeException("Phone association failed"))
        )
      ...
      phones <- client
        ...
        .catchAll(error =>
          Console.printLine(s"Failed to fetch phones: $error") *> ZIO.fail(
            new RuntimeException("Phone fetch failed")
          )
        )
  ...
  override def run: ZIO[Any, Any, Unit] =
    exampleProgram
      ...
      .catchAll(error => Console.printLine(s"Error: ${error.getMessage}").orDie)
}
